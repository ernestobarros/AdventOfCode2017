...
let deserializePassphrases (input: string) =
    input.Trim().Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries)
    |> Array.toList
    |> List.map(fun x -> Array.toList <| x.Trim().Split(' ', StringSplitOptions.RemoveEmptyEntries))

let validatePassphrase (words: string list) =
    words
    |> List.map(Seq.sort >> Seq.toArray >> (fun x -> System.String(x)) >> sprintf "%s")
    |> Set.ofList
    |> Set.count
    |> fun n -> (words.Length = n), words

[<EntryPoint>]
let main _ =
    input
    |> deserializePassphrases
    |> List.map(validatePassphrase)
    |> List.countBy(fst)
    |> List.iter(printfn "%A")
