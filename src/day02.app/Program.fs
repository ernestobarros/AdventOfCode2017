...
let toSpreadsheet (input: string) =
    input.Trim().Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries)
    |> Seq.map(fun r ->
        Regex.Split(r.Trim(), "\s+")
        |> Seq.map (int)
        |> Seq.toList)
    |> Seq.toList

let checksum (data: int list list) =
    data
    |> List.map(fun xs ->
        printfn "%A" xs
        let xs' =
        ...
        |> List.mapi(fun i x ->
            xs'
            |> List.mapi(fun i' x' -> if i = i' then 0 else x')
            |> List.filter(fun x' -> x' <> 0)
            |> List.filter((>=) x)
            |> List.sort
            |> List.filter(fun x' -> x % x' = 0)
            |> fun xs'' ->
                if xs''.Length <> 0 then printfn "%i / %A" x xs''
                xs''
            |> function
                | [] -> 0
                | x''::_ -> x / x''
...
let main _ =
    ...
    |> toSpreadsheet
    |> checksum
    |> printfn "The spreadsheet's example checksum is: %i"
    spreadsheetInput
    |> toSpreadsheet
    |> checksum
    |> printfn "The spreadsheet's checksum is: %i"
